...
class CompletenessWorkflow(AbsWorkflow):
    ...
    def create_new_task(self, task_builder: "forseti.tasks.TaskBuilder", **job_args):
        ...
        task_builder.add_subtask(subtask=HathiCheckMissingPackageFilesTask(package_path))
        task_builder.add_subtask(subtask=HathiCheckMissingComponentsTask(request_ocr_validation, package_path))
        task_builder.add_subtask(subtask=ValidateExtraSubdirectoriesTask(package_path))
        ...
        if request_ocr_validation:
            task_builder.add_subtask(subtask=ValidateOCRFilesTask(package_path))
        if request_ocr_utf8_validation:
            task_builder.add_subtask(subtask=ValidateOCFilesUTF8Task(package_path))
    ...
    def generate_report(cls, results: typing.List[forseti.tasks.Result], **user_args) -> typing.Optional[str]:
        ...
        error_results += cls._get_result(results_grouped, HathiCheckMissingPackageFilesTask)
        error_results += cls._get_result(results_grouped, HathiCheckMissingComponentsTask)
        error_results += cls._get_result(results_grouped, ValidateExtraSubdirectoriesTask)
        error_results += cls._get_result(results_grouped, ValidateChecksumsTask)
    ...
    @classmethod
    def _get_result(cls, results_grouped: typing.Dict[typing.Any, list], key) -> typing.List[hathi_result.Result]:
...
class HathiCheckMissingComponentsTask(CompletenessSubTask):
    ...
    def work(self) -> bool:
        ...
        with self.log_config(my_logger):
            ...
            missing_files_errors = validate_process.run_validation(
                validator.ValidateComponents(self.package_path, "^\d{8}$", *extensions))
            if not missing_files_errors:
                self.log("Found no missing component files in {}".format(self.package_path))
            else:
                for error in missing_files_errors:
                    self.log(error.message)
                    errors.append(error)
    ...
    def work(self) -> bool:
        ...
        with self.log_config(my_logger):
            ...
            if not extra_subdirectories_errors:
                self.log("No extra subdirectories found in {}".format(self.package_path))
            else:
        ...
        with self.log_config(my_logger):
            ...
            try:
                ...
                checksum_report_errors = validate_process.run_validation(
                    validator.ValidateChecksumReport(self.package_path, checksum_report))
        ...
        with self.log_config(my_logger):
            try:
                if not os.path.exists(marc_file):
                    ...
                    self.log("Validating marc.xml in {}".format(self.package_path))
...
class ValidateOCRFilesTask(CompletenessSubTask):
    ...
    def work(self) -> bool:
        ...
        with self.log_config(my_logger):
            ocr_errors = validate_process.run_validation(validator.ValidateOCRFiles(path=self.package_path))
            if ocr_errors:
                self.log("No validation errors found in ".format(self.package_path))
                # else:
        ...
        with self.log_config(my_logger):
            ...
            try:
                if not os.path.exists(yml_file):
                    ...
                    self.log("Validating meta.yml in {}".format(self.package_path))
                    meta_yml_errors = validate_process.run_validation(
                        validator.ValidateMetaYML(yaml_file=yml_file, path=self.package_path, required_page_data=True))
...
class ValidateOCFilesUTF8Task(CompletenessSubTask):
    ...
    def work(self) -> bool:
        ...
        with self.log_config(my_logger):
            ...
            for ocr_file in filter(filter_ocr_only, os.scandir(self.package_path)):
