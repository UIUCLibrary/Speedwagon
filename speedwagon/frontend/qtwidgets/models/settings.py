            ...
            if column == 0:
                if self.workflow is not None:
                    return self.workflow.name \
                        if self.workflow.name is not None else ''
                return ''
            return None
        return None
...
class EditSettingsDataRole:
    ...
    def display_role(self, column: int) -> SettingsDataType:
        label = self.parent.option.label \
            if self.parent.option is not None \
            else ''
        return {
            0: label,
            1: self.parent.value
...
class WorkflowSettingsRoot(AbsWorkflowSettingItem):
    ...
    def child_number(self) -> int:
        return typing.cast(
            List[AbsWorkflowSettingItem],
            self.parent_item.child_items
        ).index(self) if self.parent_item else 0
...
class WorkflowSettingsModel(QtCore.QAbstractItemModel):
    ...
    def headerData(  # pylint: disable=invalid-name
            self,
            section: int, orientation: QtCore.Qt.Orientation,
            role: int = QtCore.Qt.ItemDataRole.DisplayRole
    ) -> Any:
        if (
    ...
    def rowCount(  # pylint: disable=invalid-name
        ...
        if parent.isValid() and parent.column() > 0:
            return 0
        parent_item = \
            typing.cast(Optional[WorkflowSettingsRoot], self.get_item(parent))
    ...
    def add_workflow(self, workflow: speedwagon.Workflow) -> None:
        self.root_item.insert_children(
            self.root_item.child_count(),
            1,
            self.root_item.column_count()
        )
        workflow_item: Optional[WorkflowSettingsItemWorkflow] = \
    ...
    def results(self) -> WorkflowsSettings:
        ...
        for workflow_row_id in range(self.rowCount()):
            ...
            item = typing.cast(
                ...
                self.get_item(workflow_index)
            )
            if item.workflow is None:
                continue

            settings: SettingsData = {
                str(self.data(self.index(row, 0, parent=workflow_index))):
            ...
            workflow_name = item.workflow.name \
                if item.workflow.name is not None else ''
