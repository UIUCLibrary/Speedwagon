...
//                        tee('logs/build.log') {
                        dir("source"){
                            lock("system_pipenv_${NODE_NAME}"){
                                powershell script: "& ${tool 'CPython-3.6'}\\python.exe -m pipenv run python setup.py build -b ${WORKSPACE}\\build | tee ${WORKSPACE}\\logs\\build.log", returnStdout: false
                            }
...
//                        tee('logs/build_sphinx.log') {
                        dir("source"){
                            lock("system_pipenv_${NODE_NAME}"){
                                bat "${tool 'CPython-3.6'}\\python -m pipenv run python setup.py build_sphinx --build-dir ${WORKSPACE}\\build\\docs > ${WORKSPACE}\\logs\\build_sphinx.log & type ${WORKSPACE}\\logs\\build_sphinx.log"
                            }
                        }
//                        }
                    }
